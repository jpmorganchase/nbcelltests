...
# apparently can't ask serverextension about individual extensions (and it's OK on linux/mac but ok on windows :) )
	${PYTHON} -c "import subprocess,re,sys; import nbcelltests;  ext=subprocess.check_output(['jupyter','serverextension','list'],stderr=subprocess.STDOUT).decode();  print(ext);  res0=re.search('.*nbcelltests {} ok'.format(nbcelltests.__version__),ext,re.IGNORECASE);  res1=re.search('nbcelltests\s*enabled', ext);  sys.exit(not (res0 and res1))"
