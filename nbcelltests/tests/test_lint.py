...
@pytest.mark.parametrize(
    ...
    [
        (
            3,
            [0, 1, 2, 3, 4],
            [LR(x, LintType.LINES_PER_CELL) for x in [True, True, True, True, False]],
            False,
        ),
        (
            3,
            [0, 1, 2, 3],
            [LR(x, LintType.LINES_PER_CELL) for x in [True, True, True, True]],
            True,
        ),
        (-1, [0, 1], [], True),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"lines_per_cell": 1},
            None,
            [LR(x, LintType.LINES_PER_CELL) for x in [False, True, False, False]],
            False,
        ),
        ...
        (
            {"lines_per_cell": 1, "cells_per_notebook": 10},
            "# LINT_DISABLE: notebook_(.*)$",
...
def _check(html, expected_results):
    ...
    for actual, expected in zip(
        sorted(actual_results, key=lambda x: x.text[x.text.find("Checking ") : :]),
        sorted(expected_results, key=itemgetter(1)),
    ):
