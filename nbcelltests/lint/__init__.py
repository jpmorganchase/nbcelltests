...
def run(
    ...
    if "lines_per_cell" in extra_metadata:
        lintret, lintpassed = lint_lines_per_cell(
    ...
    if "cells_per_notebook" in extra_metadata:
        lintret, lintpassed = lint_cells_per_notebook(
    ...
    if "function_definitions" in extra_metadata:
        lintret, lintpassed = lint_function_definitions(
    ...
    if "class_definitions" in extra_metadata:
        lintret, lintpassed = lint_class_definitions(
    ...
    if "kernelspec_requirements" in extra_metadata:
        lintret, lintpassed = lint_kernelspec(
    ...
    if "magics_allowlist" in extra_metadata or "magics_denylist" in extra_metadata:
        lintret, lintpassed = lint_magics(
            magics=extra_metadata["magics"],
            allowlist=extra_metadata.get("magics_allowlist", None),
            denylist=extra_metadata.get("magics_denylist", None),
        )
        ret.extend(lintret)
        passed = passed and lintpassed
